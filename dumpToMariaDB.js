...
async function run() {
    try {
        ...
        await editAccount("B10815004@mail.ntust.edu.t", '123456');
    } catch (err) {
        console.log(err);
    } finally {
...
async function editAccount(account, password) {
    try {
        const conn = await connectDatabase();
        await conn.query("USE playerDB");

        let query = `
            SELECT COUNT(*) FROM webUser WHERE account = ?;
        `;

        const query_result = await conn.query(query, account);
        let result;
        if (query_result.count){
            query = `
                ...
                WHERE account = ?
            `;
            await conn.query(query, [password, account]);
            result = "OK";
        } else {
            result = "Failed";
        }
